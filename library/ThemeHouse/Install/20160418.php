...
class ThemeHouse_Install
{
    ...
    public static function install()
    {
        ...
        if ($data['addon_id'] != 'ThemeHouse_InstallUpgrade' && isset($xenAddons[$wainAddon]) && !isset($GLOBALS['th_upgrade'][$addOnKey]) && strpos($data['addon_id'], 'ThemeHouse_') !== false) {
        ...
        if ($installer) {
            ...
            if (empty($args[0])) {
                $addOn = XenForo_Model::create('XenForo_Model_AddOn')->getAddOnById($data['addon_id']);
                $installer->setCurrentVersion($addOn['version_id']);
                $installer->setNewVersion($addOn['version_id']);
            } else {
                if (isset($args[0]) && isset($args[0]['version_id'])) {
                    $installer->setCurrentVersion($args[0]['version_id']);
                }
                if (isset($args[1]) && isset($args[1]['version_id'])) {
                    $installer->setNewVersion($args[1]['version_id']);
                }
            }
    ...
    protected static function _getInstaller($addOnId)
    {
        ...
        } elseif (class_exists($addOnId.'_Install')) {
     ...
     * @return ThemeHouse_Install
     */
    public static function getInstaller($addOnId)
    {
        return self::_getInstaller($addOnId);
    }
    ...
    {
        return ($this->getNewVersion() > $this->getCurrentVersion());
    }

    public function isInstall()
    {
        return ($this->getCurrentVersion() ? false : true);
    }

    public function isRebuild()
    {
        return ($this->getCurrentVersion() === $this->getNewVersion() ? true : false);
    }

    final protected function _install()
    {
    ...
    final protected function _uninstall()
    {
    ...
    protected function _insertContentTypes(array $contentTypes)
    {
        ...
        foreach ($contentTypes as $contentType => $contentTypeParams) {
            ...
            if (isset($contentTypeParams['addon_id'])) {
                ...
                $fields = ThemeHouse_Helper_Php::serialize($existingFields);
     ...
     * @return bool
     */
    final protected function _isTableExists($tableName)
    {
        if (!self::$_tablesList) {
    ...
    final protected function _isAddOnInstalled($addOnId)
    {
        /* @var $addOnModel XenForo_Model_AddOn */
        $addOnModel = $this->getModelFromCache('XenForo_Model_AddOn');
     ...
     */
    final public function getTableChangesForAddOn($addOnId)
    {
     ...
     * @return array $tableChanges
     */
    final public function getTableChangesForAddOnOnInstall($addOnId)
    {
        $addOnTableChanges = $this->_getAddOnTableChanges();
     ...
     * @return array $tableChanges
     */
    final public function getTableChangesForAddOnOnUninstall($addOnId)
    {
        $addOnTableChanges = $this->_getAddOnTableChanges();
     ...
     */
    final protected function _getTableChangesForAddOn($addOnId, $tableName)
    {
     ...
     * @return array $tableChanges
     */
    final protected function _getAllTableChangesOnInstall()
    {
        /* @var $addOnModel XenForo_Model_AddOn */
     ...
     * @return array $tableChanges
     */
    final protected function _getAllTableChangesOnUninstall()
    {
        /* @var $addOnModel XenForo_Model_AddOn */
        $addOnModel = XenForo_Model::create('XenForo_Model_AddOn');
        $addOns = $addOnModel->getAllAddOns();
